...
class GraphRenderer {
    constructor(canvasElement)
    {
        ...
        this._animationDuration = 2 * millisecondsPerSecond;
    }
    ...
    #updateRendering()
    {
        ...
        if (this._parameters.animate) {
            requestAnimationFrame(() => {
                this._parameters.superEllipseS = this.#sForTime(performance.now());
                this.#updateRendering();
            });
        }
    }
    ...
    #sForTime(now)
    {
        const timeSinceStart = now - this._animationStartTime;

        const intervalIntervalCount = Math.floor(timeSinceStart / this._animationDuration);
        
        const intervalTime = timeSinceStart % this._animationDuration;
        let intervalFraction = intervalTime / this._animationDuration;
...
class WindowController {
    ...
    #connectControls()
    {
        ...
        setupSlider('animation-fraction', 'animationFraction');
